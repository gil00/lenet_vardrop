...
def create_dataset(opt):
    ...
    train_transform = transforms.Compose( [ 
        transforms.RandomCrop(32, padding=opt.randomcrop_pad),
        transforms.RandomHorizontalFlip(),
        convert,])
    trainset = ds( root=opt.dataroot, train=True, download=True, transform=train_transform )
    ...
    testset = ds( root=opt.dataroot, train=False, download=True, transform=convert)
...
def main() :
    ...
    if opt.resume_path != '':
        ...
        import time
        timetag = time.strftime("%Y_%m%d_%H%M%S")
