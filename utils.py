...
def dropoutstats(rate):
    return cast({'rate': torch.mul(torch.ones(1), rate)})

def data_parallel(f, input, params, stats, mode, device_ids, dropout_methods = 'simple_notfix', output_device=None):
    if output_device is None:
        output_device = device_ids[0]

    if len(device_ids) == 1:
        return f(input, params, stats, mode, dropout_methods)
